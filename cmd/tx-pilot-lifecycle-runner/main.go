...
func main() {
	...
	baseURL := flag.String("base", "", "API base URL (defaults to TX_PILOT_PUBLIC_API_BASE_URL)")
	flag.Parse()
...
func postJSON(ctx context.Context, url string, payload any) (txpilot.SubmitResponse, error) {
	...
	if resp.StatusCode >= 300 {
		return txpilot.SubmitResponse{}, fmt.Errorf("status %d: %s", resp.StatusCode, string(body))
	}
	var out txpilot.SubmitResponse
