...
func main() {
	var (
		baseURL = flag.String("url", "http://localhost:8080", "TX Pilot API base URL")
		memo    = flag.String("memo", "tx-pilot-cli", "memo")
	...
	if cfg.KeypairPath == "" {
		fmt.Fprintln(os.Stderr, "TX_PILOT_KEYPAIR_PATH required for CLI signing")
		os.Exit(1)
	}
	...
	encodedTransfer, err := tx.EncodeTransaction(transfer, txpilot.EncodingBase64)
	if err != nil {
	...
	if *bundle {
		...
		encodedTip, err := tx.EncodeTransaction(tipTx, txpilot.EncodingBase64)
		if err != nil {
			fmt.Fprintf(os.Stderr, "encode tip: %v\n", err)
			os.Exit(1)
		}
		body, _ := json.Marshal(txpilot.SubmitBundleRequest{
	...
	body, _ := json.Marshal(txpilot.SubmitTransactionRequest{
